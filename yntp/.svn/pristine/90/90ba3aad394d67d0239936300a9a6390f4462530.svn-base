...
define(function(require, exports, module) {
	...
	var FORMOPTURL_INSERT= GLOBAL.BASE + "admin/car/cartype/create";
	var FORMOPTURL_UPDATE= GLOBAL.BASE + "admin/car/cartype/update";
	
	var TARGETDIV_FORM = "basearchive_form";
	var MODULENAME = "car_type";
	
	var COMPONENTID_FORM = "validationform" + "_" + MODULENAME;
	
	var tableUrl = GLOBAL.HTML_HOME + "admin/sys/car/sys_car_type_table.html";
	...
	function initFormValue(optType,data){
		...
		if (optType == basearchiveUtil.FORMOPT_CREATE) {
			...
			// 重新从数据库中读取数据
			var queryUrl = GLOBAL.BASE +  "admin/car/cartype/search?q=id=" + data.id;
			var lastdata = null;
			$.ajax({
				async : false,
				type : "GET",
