...
define(function(require, exports, module) {
	...
	var init = function(param) {
		/** 查询景点，在tabbed内部显示*/
		var nationalspecial_query = GLOBAL.BASE + "front/nationalspecial/nationalspecial/frontshow/fileds/pagesearch?start=0&size=7&q=";
    			...
    			success : function(resultData) {
    				if('000000' == resultData.flag &&  resultData.data != null && resultData.data.items != null && resultData.data.items.length > 0){
						...
						for (var i = 0; i < dataItems.length; i++) {
							...
							listBlockOut += '<li style="height:80px;vertical-align:middle;">';
							listBlockOut +=     '<div class="avatar pull-left" style="height:80px;">';
							listBlockOut +=         '<span class="label label-important">' + (i+1) + '</span>';
							listBlockOut +=     '</div>';
							listBlockOut +=     '<div class=" ">';
