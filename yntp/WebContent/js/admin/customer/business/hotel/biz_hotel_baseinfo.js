...
define(function(require, exports, module) {
	...
	var FORMOPTURL_QUERY = GLOBAL.BASE + "admin/business/hotel/search";

	var MODULENAME = "hotel_baseinfo";
	...
	var initFormComponent = function() {
		basearchiveUtil.initSelect2Value({
			queryUrl:GLOBAL.BASE + "admin/hotel/hoteltype/getselectmodel",
		...
		basearchiveUtil.initSelect2Value({
			queryUrl:GLOBAL.BASE + "admin/hotel/hotellevel/getselectmodel",
	...
	function initFormValue(optType, data) {
		if (optType == basearchiveUtil.FORMOPT_CREATE) { //新增，清空数据
				...
				success : function(resultdata) {
					if ('000000' != resultdata.flag) {
						...
						if (receiptList != null && receiptList.length > 0) {
							...
							store.set("currentSelectRowDatahotel",lastdata);
						} else {
							bootbox.alertTimeout('查询数据出现错误，请刷新重试！');
			...
			if (com_abstract != 'undefined' && com_abstract != null)
				com_abstract.attr("value", currentLastData.hotel_abstract);
	...
	exports.getFormData = function() {
		...
		var formData = {
				...
				hotel_abstract : com_abstract.attr("value"),
