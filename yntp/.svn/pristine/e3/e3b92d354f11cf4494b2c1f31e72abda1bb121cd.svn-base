...
define(function(require, exports, module) {
	...
	exports.init = function(param) {
		/** 查询景点，在tabbed内部显示*/
		var scenic_query = GLOBAL.BASE + "front/scenic/scenic/frontshow/fileds/pagesearch?start=0&size=8";
		scenic_query += "&q=type_id=" + param.type_id;
    				...
    				if('000000' == resultData.flag &&  resultData.data != null && resultData.data.items != null && resultData.data.items.length > 0){
    					...
    					var dataItems = dataList;
    					  for(var i = 0;i < dataItems.length; i++){
    						  ...
    						  listBlockOut = listBlockOut +                     '<span>' +(dataItem.minTicketPrice == null ? "门票暂无" :  ('¥' + dataItem.minTicketPrice+ '起')) + '</span>';
    						  listBlockOut = listBlockOut +                 '</div>';
    						  listBlockOut = listBlockOut +             '</div>';
    						  listBlockOut = listBlockOut +         '</div>';
    						  listBlockOut = listBlockOut +     '</div>';
		...
		// 实例化组件-------------------------START
		var template = require("./sceniclist.tpl");
