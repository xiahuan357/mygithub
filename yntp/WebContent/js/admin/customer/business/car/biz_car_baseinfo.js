...
define(function(require, exports, module) {
	...
	var FORMOPTURL_QUERY = GLOBAL.BASE + "admin/business/car/search";

	var MODULENAME = "car_baseinfo";
	...
	var initFormComponent = function() {
		basearchiveUtil.initSelect2Value({
			queryUrl:GLOBAL.BASE + "admin/car/cartype/getselectmodel",
		...
		basearchiveUtil.initSelect2Value({
			queryUrl:GLOBAL.BASE + "admin/car/carbrand/getselectmodel",
			com_select2 : com_brand,
			placeholder : "汽车品牌",
			errorMsg : "初始化汽车品牌失败",
		});
	}
	...
	function initFormValue(optType, data) {
		if (optType == basearchiveUtil.FORMOPT_CREATE) { //新增，清空数据
				...
				success : function(resultdata) {
					if ('000000' != resultdata.flag) {
						...
						if (receiptList != null && receiptList.length > 0) {
							...
							store.set("currentSelectRowDatacar",lastdata);
						} else {
							bootbox.alertTimeout('查询数据出现错误，请刷新重试！');
			...
			if (com_abstract != 'undefined' && com_abstract != null)
				com_abstract.attr("value", currentLastData.car_abstract);
			
			if (com_type != 'undefined' && com_type != null)
				com_type.select2("val", currentLastData.type_id);

			if (com_brand != 'undefined' && com_brand != null)
				com_brand.select2("val", currentLastData.brand_id);
		}
	}
