...
define(function(require, exports, module) {
	...
	var FORMOPTURL_INSERT= GLOBAL.BASE + "admin/equipment/equipmenttype/create";
	var FORMOPTURL_UPDATE= GLOBAL.BASE + "admin/equipment/equipmenttype/update";
	
	var TARGETDIV_FORM = "basearchive_form";
	var MODULENAME = "equipment_type";
	
	var COMPONENTID_FORM = "validationform" + "_" + MODULENAME;
	
	var tableUrl = GLOBAL.HTML_HOME + "admin/sys/equipment/sys_equipment_type_table.html";
	...
	function initFormValue(optType,data){
		...
		if (optType == basearchiveUtil.FORMOPT_CREATE) {
			...
			// 重新从数据库中读取数据
			var queryUrl = GLOBAL.BASE +  "admin/equipment/equipmenttype/search?q=id=" + data.id;
			var lastdata = null;
			$.ajax({
				async : false,
				type : "GET",
