...
define(function(require, exports, module) {
	...
	var formUrl = GLOBAL.HTML_HOME + "admin/sys/region/sys_region_form_sec.html";
	var MODULENAME = "region";
	var tableUrl_next = GLOBAL.HTML_HOME + "admin/sys/region/sys_region_thr.html";
	var tableUrl_last = GLOBAL.HTML_HOME + "admin/sys/region/sys_region_fir.html";
	var param = {
		// Table信息
		table_attr : {
			...
			},
			{
				...
				"mData" : null,
				"mRender" : function(data, type, full) {
					return '<input id="butten_last" class="btn btn-link" type="button" value="上一级" style="margin-bottom:5px">'
					+ '<input id="butten_next" class="btn btn-link" type="button" value="下一级" style="margin-bottom:5px">';
				}
			},
			{
				"aTargets" : [ 3 ],
	...
	exports.init = function(){
		$(document).ready(function() {
			var selectId_one = store.get('region_one');
			param.table_attr.requesturl = GLOBAL.BASE + "admin/region/region/pagesearch?q=parent_id=" +selectId_one;
			// 初始化table
			table.init(param);
			...
			//Table页面跳转到上一级
			$('#butten_last').die().live('click',function(e){
				
				basearchiveUtil.jumpToHtml(tableUrl_last);
			});
			...
			//Table页面跳转到下一级
			$('#butten_next').die().live('click',function(e){
				...
				store.set("region_two",selectId);
				basearchiveUtil.jumpToHtml(tableUrl_next);
			});
