 ...
 */
@Component
public class HotelTypeValidate extends DefaultValidate<HotelTypeEntity, Long> {
  @Autowired
  ...
  public ReturnInfo validateCreate(Object... param) {
    ...
    List<HotelTypeEntity> resultList = hotelTypeService.query(parsedQuery);
  ...
  public ReturnInfo validateDelete(Object... param) {
    ...
    if (resultList != null && !resultList.isEmpty()) {
      rtn_info.setFlag(false);
      rtn_info.append("该酒店类型已被使用，不能删除");
    }
    return rtn_info;
  }
}
