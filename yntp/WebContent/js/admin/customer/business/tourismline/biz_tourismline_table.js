...
define(function(require, exports, module) {
	...
	var FORMOPTURL_DELETE = GLOBAL.BASE + "admin/business/tourismline/deletewithimage/";
	
	var TARGETDIV_TABLE = "basearchive_table";
	
	var COMPONENTID_TABLE = "pagedatatable";
	var formUrl = GLOBAL.HTML_HOME + "admin/customer/business/tourismline/biz_tourismline_form.html";
	var tableUrl_trip = GLOBAL.HTML_HOME + "admin/customer/business/tourismline/biz_tourismline_trip_table.html";
	var MODULENAME = "tourismline";
		...
		table_attr : {
			...
			},
			{
				...
				"mRender" : function(data, type, full) {
					...
					+ '<a href="#" class="btn contrast icon-reorder table_btn_tripedit" title="行程编辑" style="margin-left:10px;">行程编辑</a>';
				}
			}]
		}
	};
	...
	exports.init = function(){
		$(document).ready(function() {
			...
			var TABLEOPTURL_QUERY = GLOBAL.BASE + "admin/business/tourismline/pagesearch/"+userInfo.id+"/tourismlinelist";
			...
			// 行程编辑
			$('.table_btn_tripedit').die().live('click',function(e) {
				// 修改form的type
				var datatable = $('#'+param.table_attr.tableid);
				...
				store.set("currentOptType" + MODULENAME, basearchiveUtil.FORMOPT_UPDATE);
				store.set("currentSelectRowData" + MODULENAME, rowData);
				basearchiveUtil.jumpToHtml(tableUrl_trip);
			});
