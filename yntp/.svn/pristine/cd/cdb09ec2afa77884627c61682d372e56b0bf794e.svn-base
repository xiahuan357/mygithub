package com.yn.yntp.module.region.validate;
...
@Component
public class RegionValidate extends DefaultValidate<RegionEntity, Long> {
  @Autowired
  ...
  public ReturnInfo validateCreate(Object... param) {
    ...
    parsedQuery.add(new ImmutableTriple<String, ClientOperation, String>(
        "parent_id", ClientOperation.EQ, String.valueOf(currentEntity
            .getParent_id())));
    List<RegionEntity> resultList = regionService.query(parsedQuery);
    if (resultList != null && !resultList.isEmpty()) {
      rtn_info.setFlag(false);
      rtn_info.append("已存在同名的地区");
    }
    return rtn_info;
  }
  ...
  public ReturnInfo validateDelete(Object... param) {
    ...
    List<Triple<String, ClientOperation, String>> parsedQuery =
        new ArrayList<Triple<String, ClientOperation, String>>();
    parsedQuery.add(new ImmutableTriple<String, ClientOperation, String>(
        "parent_id", ClientOperation.EQ, String.valueOf(id)));
    List<RegionEntity> resultList = regionService.query(parsedQuery);
    if (resultList != null && !resultList.isEmpty()) {
      rtn_info.setFlag(false);
      rtn_info.append("该项存在下级城市，不能删除，如果要删除，请先删除完下级城市！");
    }
    return rtn_info;
  }
