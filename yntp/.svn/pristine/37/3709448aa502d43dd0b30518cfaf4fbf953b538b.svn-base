...
define(function(require, exports, module) {
	...
	var FORMOPTURL_QUERY = GLOBAL.BASE + "admin/news/news/search";
	var FORMOPTURL_INSERT= GLOBAL.BASE + "admin/news/news/createwithimage";
	var FORMOPTURL_UPDATE= GLOBAL.BASE + "admin/news/news/updatewithimage";
	...
	var com_newstype = $('#text_newstypeid');
	...
	var initFormComponent = function() {
		basearchiveUtil.initSelect2Value({
			queryUrl:GLOBAL.BASE + "admin/news/newstype/getselectmodel",
	...
	var fnsubmithandler = function(){
		...
		var vnewstype = com_newstype.val(); //新闻类别
		...
		if(vnewstype == null || vnewstype == "" || $.trim(vnewstype) == ""){
			bootbox.alertTimeout("请选择新闻分类！");
			return;
		}
		...
		var requestURL;
		var requestType;
		var userEntity = userutil.getUserInfo();
		...
		if (optType == basearchiveUtil.FORMOPT_UPDATE) {
			newsEntity.id=vid;
			newsEntity.createtime = new Date(currentLastData.createtime).format("yyyy-MM-dd hh:mm:ss");
	...
	function initFormValue(optType,data){
		...
		if (optType == basearchiveUtil.FORMOPT_CREATE) { //新增，清空数据
			...
			if (com_abstract != 'undefined' && com_abstract != null)
				com_abstract.val(data.news_abstract);
			
			initUeditor(data.news_content);  //初始化文本内容
			// 将记录的上传的文件清除
