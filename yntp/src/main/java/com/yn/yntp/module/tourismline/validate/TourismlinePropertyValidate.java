 ...
 */
@Component
public class TourismlinePropertyValidate extends
    DefaultValidate<TourismlinePropertyEntity, Long> {

  @Autowired
  private TourismlinePropertyService tourismlinePropertyService;
  ...
  public ReturnInfo validateCreate(Object... param) {
    ...
    List<TourismlinePropertyEntity> resultList =
        tourismlinePropertyService.query(parsedQuery);
    if (resultList != null && !resultList.isEmpty()) {
      rtn_info.setFlag(false);
      rtn_info.append("已存在同名的线路性质");
    }
    return rtn_info;
  }
  ...
  public ReturnInfo validateDelete(Object... param) {
    ...
    List<Triple<String, ClientOperation, String>> parsedQuery =
        new ArrayList<Triple<String, ClientOperation, String>>();
    parsedQuery.add(new ImmutableTriple<String, ClientOperation, String>(
        "property_id", ClientOperation.EQ, String.valueOf(id)));

    List<TourismlineEntity> resultList = tourismlineService.query(parsedQuery);
    if (resultList != null && !resultList.isEmpty()) {
      rtn_info.setFlag(false);
      rtn_info.append("该线路性质已被使用，不能删除");
    }
    return rtn_info;
  }
