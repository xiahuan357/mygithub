...
define(function(require, exports, module) {
	...
	var typeUrl = GLOBAL.HTML_HOME + "admin/sys/news/sys_news_type_select_form.html";
	var tableUrl = GLOBAL.HTML_HOME + "admin/sys/news/sys_news_type_table.html";
		...
		table_attr : {
			...
			},
			{
				...
				"mRender" : function(data, type, full) {
					...
					+ '<a href="#" class="btn contrast icon-laptop table_btn_info" title="预览" style="margin-left:10px">预览</a>'
					+ '<a href="#" class="btn contrast icon-laptop table_btn_mod" title="修改该新闻类型" style="margin-left:10px;margin-right:100px">修改类型</a>';
					}
			}]
		}
	};
	...
	exports.init = function(){
		$(document).ready(function() {
			...
			var TABLEOPTURL_QUERY = "querynewstitle.action?type="+type+"&t="+ (new Date()).valueOf();
			...
			$('.table_btn_info').die().live('click', function() {
				...
				var currentUrl=GLOBAL.HTML_HOME+"admin/sys/news/sys_news_approve_chief.html";
				store.set("currentUrl",currentUrl);
				basearchiveUtil.jumpToHtml(infoUrl);
			});
			$('.table_btn_mod').die().live('click', function() {
				// 修改新闻的type
				var datatable = $('#'+param.table_attr.tableid);
