...
define(function(require, exports, module) {
	...
	var MODULENAME = "food_imagelibrary";

	var ImageLibrary = require("imagelibrary");

	var foodImageLibrary;
	// 初始化Form组件
	var initFormComponent = function(wrapperDivId) {
		 foodImageLibrary = new ImageLibrary({
			 attrs : {
				 editorid : "food_imagelibrary",
			 },
	...
	var initFormValue = function(optType, data) {
		...
		if (optType == basearchiveUtil.FORMOPT_CREATE) { //新增，清空数据
			getImagelibrarykey();
			var imagelibrarykey = store.get("foodimagelibrarykey");
			foodImageLibrary.initUploadUEditor("food",imagelibrarykey);
			foodImageLibrary.initImageLibrary(basearchiveUtil.FORMOPT_CREATE,imagelibrarykey);
		} else if (optType == basearchiveUtil.FORMOPT_UPDATE) { //修改，回填数据
			var imagelibrarykey = data.imagelibrarykey;
			store.set("foodimagelibrarykey",imagelibrarykey);
			foodImageLibrary.initUploadUEditor("food",imagelibrarykey);
			foodImageLibrary.initImageLibrary(basearchiveUtil.FORMOPT_UPDATE,imagelibrarykey,data.coverimagename);
		}
	}
	...
	function getImagelibrarykey(){
		$.ajax({
	        ...
	        {
	        	store.set("foodimagelibrarykey",data.data);
	        },
	...
	exports.getFormData = function() {
		var formData = {
			imagelibrarykey : store.get("foodimagelibrarykey"),
			coverimagename : foodImageLibrary.getCoverFileName(),
		}
		return formData;
	}
