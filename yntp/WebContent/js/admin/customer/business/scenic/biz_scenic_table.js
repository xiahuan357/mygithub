...
define(function(require, exports, module) {
	...
	var FORMOPTURL_DELETE = GLOBAL.BASE + "admin/business/scenic/deletewithimage/";
	...
	var tableUrl_ticket = GLOBAL.HTML_HOME + "admin/customer/business/scenic/biz_scenic_ticket_table.html";
		...
		table_attr : {
			...
			},
			{
				...
				"mRender" : function(data, type, full) {
					...
					+ '<a href="#" class="btn contrast icon-reorder table_btn_ticketedit" title="门票编辑" style="margin-left:10px;">门票编辑</a>';
				}
			}]
		}
	};
	...
	exports.init = function(){
		$(document).ready(function() {
			...
			var TABLEOPTURL_QUERY = GLOBAL.BASE + "admin/business/scenic/pagesearch/"+userInfo.id+"/sceniclist";
			...
			// 门票编辑
			$('.table_btn_ticketedit').die().live('click',function(e) {
				// 修改form的type
				var datatable = $('#'+param.table_attr.tableid);
				...
				store.set("currentOptType" + MODULENAME, basearchiveUtil.FORMOPT_UPDATE);
				store.set("currentSelectRowData" + MODULENAME, rowData);
				basearchiveUtil.jumpToHtml(tableUrl_ticket);
			});
						...
						$.ajax({
							...
							success : function(resultdata) {
								if('000000' != resultdata.flag){
									bootbox.confirm('删除失败，请重试，错误原因：' +resultdata.desc );
								} else {
									// 显示成功提交
