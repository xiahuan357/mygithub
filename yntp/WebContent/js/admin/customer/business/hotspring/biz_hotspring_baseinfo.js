...
define(function(require, exports, module) {
	...
	var FORMOPTURL_QUERY = GLOBAL.BASE + "admin/business/hotspring/search";

	var MODULENAME = "hotspring_baseinfo";
	...
	var initFormComponent = function() {
		basearchiveUtil.initSelect2Value({
			queryUrl:GLOBAL.BASE + "admin/hotspring/hotspringtype/getselectmodel",
		...
		basearchiveUtil.initSelect2Value({
			queryUrl:GLOBAL.BASE + "admin/hotspring/hotspringlevel/getselectmodel",
	...
	function initFormValue(optType, data) {
		if (optType == basearchiveUtil.FORMOPT_CREATE) { //新增，清空数据
				...
				success : function(resultdata) {
					if ('000000' != resultdata.flag) {
						...
						if (receiptList != null && receiptList.length > 0) {
							...
							store.set("currentSelectRowDatahotspring",lastdata);
						} else {
							bootbox.alertTimeout('查询数据出现错误，请刷新重试！');
			...
			if (com_abstract != 'undefined' && com_abstract != null)
				com_abstract.attr("value", currentLastData.hotspring_abstract);
	...
	exports.getFormData = function() {
		...
		var formData = {
				...
				hotspring_abstract : com_abstract.attr("value"),
