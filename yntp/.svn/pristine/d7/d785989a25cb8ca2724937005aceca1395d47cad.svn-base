...
/**
 * 
 * @Title: HotelRoomValidate.java 
 * @Package com.yn.yntp.module.hotel.validate 
 * @Description: TODO(用一句话描述该文件做什么) 
 *
 * @author liucc    
 * @date 2015年1月16日 下午2:44:52 
 * @version V1.0 
 */

@Component
public class HotelRoomValidate extends DefaultValidate<HotelRoomEntity, Long> {
  ...
  public ReturnInfo validateCreate(Object... param) {
    ...
    parsedQuery.add(new ImmutableTriple<String, ClientOperation, String>(
        "hotelRoomTypeEntity.id", ClientOperation.EQ,String.valueOf(currentEntity.getHotelRoomTypeEntity().getId())));
    parsedQuery.add(new ImmutableTriple<String, ClientOperation, String>(
        "hotel_id", ClientOperation.EQ,String.valueOf(currentEntity.getHotel_id())));

    List<HotelRoomEntity> resultList = hotelRoomService.query(parsedQuery);
    if (resultList != null && !resultList.isEmpty()) {
      rtn_info.setFlag(false);
      rtn_info.append("已存在同类型的房间");
    }
    return rtn_info;
  }
