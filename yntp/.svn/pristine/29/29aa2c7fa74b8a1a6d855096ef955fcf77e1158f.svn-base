...
define(function(require, exports, module) {
	...
	exports.init = function(param) {
		/** 查询旅行设备，在tabbed内部显示*/
		var equipment_query = GLOBAL.BASE + "front/equipment/equipment/frontshow/fileds/pagesearch?start=0&size=8";
		equipment_query += "&q=type_id=" + param.type_id;
        handlebars.registerHelper('equipmentlist', function(items, options) {
        	var dataList;
    		var listBlockOut = "";
            $.ajax({
            	async : false,
    			...
    			success : function(resultData) {
    				if('000000' == resultData.flag &&  resultData.data != null && resultData.data.items != null && resultData.data.items.length > 0){
    					...
    					listBlockOut = "还未有旅行设备，后续会添加。。敬请期待！"
    				}
    			},
		...
		// 实例化组件-------------------------START
		var template = require("./equipmentlist.tpl");
